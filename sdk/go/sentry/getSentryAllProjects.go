...
func GetSentryAllProjectsOutput(ctx *pulumi.Context, args GetSentryAllProjectsOutputArgs, opts ...pulumi.InvokeOption) GetSentryAllProjectsResultOutput {
	return pulumi.ToOutputWithContext(context.Background(), args).
		ApplyT(func(v interface{}) (GetSentryAllProjectsResultOutput, error) {
			args := v.(GetSentryAllProjectsArgs)
			opts = internal.PkgInvokeDefaultOpts(opts)
			var rv GetSentryAllProjectsResult
			secret, err := ctx.InvokePackageRaw("sentry:index/getSentryAllProjects:getSentryAllProjects", args, &rv, "", opts...)
			if err != nil {
				return GetSentryAllProjectsResultOutput{}, err
			}

			output := pulumi.ToOutput(rv).(GetSentryAllProjectsResultOutput)
			if secret {
				return pulumi.ToSecret(output).(GetSentryAllProjectsResultOutput), nil
			}
			return output, nil
